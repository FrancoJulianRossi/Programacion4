...
describe("Visualizacion inicial del pedido", () => {
  test("HU2 - Simular click sobre el boton agregar de un producto", async () => {
    render(
      <OrderProvider>
        <Menu />
        <Order />
  ...
  test("HU5 - envía el pedido y muestra mensaje de confirmación", async () => {
    render(
      <OrderProvider>
        <Menu />
        <Order />
    ...
    const orderList = screen.getByRole("list", { name: /pedido/i });
    await waitFor(() => {
      const itemsInOrder = within(orderList).queryAllByRole("listitem");
      expect(itemsInOrder.length).toBe(0);
    });
  });
});